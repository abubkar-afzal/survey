 ...
 viewallBlog = "sm:text-[---c4] sm:bg-[---c9]"
   ...
   viewallBlog = "sm:text-[---c4] sm:bg-[---c9]"
    ...
    const onDelete= async(e)=>{
      setLoader(true);
    e.preventDefault();
      let res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/deleteBlog`,{
        method:'POST',
        headers: {
  ...
  return (
    <><Fade cascade>
     {loader ? (
           ...
           <br />
           <br />
               <p className="font-bold sm:text-[18px] mm:text-[18px] lm:text-[20px] t:text-[22px] l:text-[27px] ll:text-[32px] k:text-[37px]" >Deleting Blog Please Wait !!</p>
           </div></Fade>
